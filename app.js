...
const NOMBRES_DECISION = Object.freeze({
  ...
  capacidad_bokashi: "Separación y aplicación orgánica",
});

const ID_EDITORES = Object.freeze([
  "composition-editor",
  "conversion-advanced-editor",
  "pet-editor",
  "pet-advanced-editor",
  "organic-editor",
  "organic-advanced-editor",
  "participation-editor",
  "feedback-advanced-editor",
]);
...
function crearEditorCalendario(ruta, segmento) {
  ...
  agregar.addEventListener("click", () => {
    ...
    const ultimo = calendario.length > 0 ? calendario[calendario.length - 1].hasta : 0;
...
function renderEditores() {
  ID_EDITORES.forEach((id) => $(`#${id}`).replaceChildren());
  renderPersonas();
...
function sincronizarJsonArbol() {
  ...
  if (preview) {
    let rutas = [state.arbol.raiz ?? "base"];
    state.arbol.puntos.forEach((punto) => {
      rutas = rutas.flatMap((ruta) => punto.alternativas.map((alternativa) => `${ruta}/${alternativa.etiqueta}`));
    });
    const limite = 20;
    preview.textContent = [
      ...rutas.slice(0, limite),
      ...(rutas.length > limite ? [`… y ${rutas.length - limite} rutas más`] : []),
    ].join("\n");
  }
  actualizarConteoEscenarios();
}
    ...
    punto.alternativas.forEach((alternativa, alternativaIndice) => {
      ...
      if (alternativaIndice > 0) {
        ...
        addChange.addEventListener("click", () => {
          const disponibles = caminosEscenarioPermitidos().filter(
            (ruta) => !Object.prototype.hasOwnProperty.call(alternativa.overrides, ruta),
          );
...
function invalidarResultados() {
  if (!state.reporte && !state.resultadoArbol) return;
  state.resultadoArbol = null;
  state.reporte = null;
  state.parametrosCorrida = null;
  const botonResultados = $('[data-go-step="9"]');
  if (botonResultados) botonResultados.disabled = true;
}
...
function reducirMuestras(muestras, maximo = 480) {
  ...
  if (reducidas[reducidas.length - 1] !== ultimaMuestra) reducidas.push(ultimaMuestra);
  return reducidas;
}
...
function instalarEventos() {
  const shell = $("#parameter-editor");
  const selectorQueInvalida = [
    "[data-param-path]",
    "[data-provenance-control]",
    "[data-source-control]",
    "[data-calendar-path] input",
    "[data-calendar-path] select",
    "#t-fin",
    "#reference-date",
    "#research-question",
    "#tree-spec",
  ...
  const invalidarPorEdicion = (evento) => {
    if (evento.target.matches?.(selectorQueInvalida)) invalidarResultados();
  };
  shell.addEventListener("input", invalidarPorEdicion);
  shell.addEventListener("change", invalidarPorEdicion);
  shell.addEventListener("click", (evento) => {
    if (evento.target.closest?.("[data-calendar-path] button, #scenario-list button, #add-decision, #apply-tree-json, #restore-tree, #restore-params, #use-example, #use-own-data")) {
      invalidarResultados();
    }
  });
  ...
  $("#restore-tree").addEventListener("click", () => {
    state.arbol = clonarJson(ARBOL_EJEMPLO);
    state.arbol.puntos.forEach((punto) => {
      punto.nombreVisible = punto.nombreVisible ?? NOMBRES_DECISION[punto.nombre] ?? humanizar(punto.nombre);
    });
    renderEscenarios();
  ...
  $("#restore-params").addEventListener("click", () => {
    state.parametrosBase = crearParametros();
    renderEditores();
    actualizarResto();
    actualizarPeriodo();
    actualizarPreviews();
  ...
  $("#integrator-method").addEventListener("change", () => {
    $("#method-pill").textContent = $("#integrator-method").value.toUpperCase();
  });
...
function iniciar() {
  ...
  $("#method-pill").textContent = CONFIGURACION_EJEMPLO.metodo.toUpperCase();
  renderEditores();
  actualizarResto();
  renderEscenarios();
  instalarEventos();
  ...
  if (window.location.hash === "#metodo") {
    irAPaso(8, { forzar: true });
    $("#compatibility-controls").open = true;
  }
}
...
try {
  ...
  if (box) {
    box.hidden = false;
    box.textContent = `No se pudo iniciar la interfaz. ${error?.message ?? String(error)}`;
  }
  console.error(error);
}
